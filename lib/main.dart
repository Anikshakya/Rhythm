...
class SleepTimerButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final controller = Get.find<SleepTimerController>(); // Use find; put in init
    final screenHeight = MediaQuery.of(context).size.height;
    const percentage = 1.0; // Assuming full opacity; adjust if needed from your context
  ...
  Future<Duration?> _showSleepTimerDialog(BuildContext parentContext) async {
    ...
    if (controller.isActive.value) {
      ...
      selectedIndex = options.indexWhere((opt) => opt['duration'] != null && (opt['duration'] as Duration).inMinutes == currentMin);
                ...
                children: [
                      ...
                      CupertinoButton(
                        ...
                        onPressed: () async {
                          ...
                          if (opt['duration'] == null) {
                            selectedDuration = await _showCustomTimerPicker(builderContext);
                          } else {
                  ...
                  Expanded(
                    child: CupertinoPicker(
                      ...
                      onSelectedItemChanged: (int index) => setState(() => selectedIndex = index),
                      children: options.map((opt) => Center(child: Text(opt['label'] as String))).toList(),
                    ),
                  ),
                ],
              ),
  ...
  Future<Duration?> _showCustomTimerPicker(BuildContext parentContext) async {
    final controller = Get.find<SleepTimerController>();
    Duration selectedDuration = controller.remaining.value != Duration.zero ? controller.remaining.value : const Duration(minutes: 30);
              ...
              Row(
                ...
                children: [
                  ...
                  CupertinoButton(
                    ...
                    onPressed: () => Navigator.pop(dialogContext, selectedDuration),
                  ),
                ],
              ),
              Expanded(
